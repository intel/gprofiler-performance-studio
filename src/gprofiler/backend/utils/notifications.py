...
from gprofiler.backend.config import DEFAULT_SLACK_CHANNELS, SLACK_BOT_TOKEN, SLACK_CHANNELS  # type: ignore
...
class SlackNotifier:
    ...
    def send_to_all_channels(self, text: str, thread_ts: Optional[str] = None, **kwargs) -> List[Dict[str, Any]]:
        """
        ...
        for channel in available_channels:
            try:
                response = self.send_message(text=text, channel=channel, thread_ts=thread_ts, **kwargs)
        ...
        for channel in available_channels:
            try:
                response = self.send_rich_message(
                    blocks=blocks, attachments=attachments, text=text, channel=channel, thread_ts=thread_ts, **kwargs
                )
                responses.append(response)
            except SlackApiError as e:
