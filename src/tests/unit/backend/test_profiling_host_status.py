...
class TestProfilingHostStatusEndpoint:
    ...
    def test_filter_by_service_name(
        ...
        for host in result:
            assert "devapp" in host["service_name"].lower(), "Host service_name should contain 'devapp'"
    ...
    def test_filter_by_hostname_partial(
        ...
        # All returned hosts should match the filter
        for host in result:
            assert "restricted" in host["hostname"].lower(), "Host hostname should contain 'restricted'"
    ...
    def test_filter_by_ip_address_prefix(
        ...
        # All returned hosts should match the filter
        for host in result:
            assert host["ip_address"].startswith("10.9"), "Host IP should start with '10.9'"
    ...
    def test_filter_by_profiling_status_sent(
        ...
        for host in result:
            assert host["profiling_status"] == "sent", "Host status should be 'sent'"
    ...
    def test_filter_by_profiling_status_stopped(
        ...
        for host in result:
            assert host["profiling_status"] == "stopped", "Host status should be 'stopped'"
            assert host["command_type"] == "N/A", "Stopped hosts should have command_type 'N/A'"
    ...
    def test_filter_by_command_type(
        ...
        for host in result:
            assert host["command_type"] == "start", "Host command_type should be 'start'"
    ...
    def test_combined_filters_service_and_hostname(
        ...
        for host in result:
            assert "devapp" in host["service_name"].lower(), "Should match service filter"
            assert "restricted" in host["hostname"].lower(), "Should match hostname filter"
    ...
    def test_combined_filters_service_ip_status(
        ...
        for host in result:
            assert "devapp" in host["service_name"].lower(), "Should match service filter"
            assert host["ip_address"].startswith("10.9"), "Should match IP filter"
            assert host["profiling_status"] == "sent", "Should match status filter"
    ...
    def test_exact_match_mode(
        ...
        for host in result:
            assert host["hostname"] == "devrestricted-achatharajupalli", "Should be exact match"
    ...
    def test_multiple_service_names(
        ...
        for host in result:
            ...
            assert "devapp" in service or "web-service" in service, "Should match one of the service names"
...
class TestProfilingHostStatusDataValidation:
    ...
    def test_pids_array_structure(
        ...
        result = response.json()
        for host in result:
            assert isinstance(host["pids"], list), "pids should be an array"
            # All elements should be integers
            for pid in host["pids"]:
                assert isinstance(pid, int), "PIDs should be integers"
    ...
    def test_command_type_valid_values(
        ...
        for host in result:
            ...
            assert (
                cmd_type in valid_command_types
    ...
    def test_active_hosts_have_valid_command(
        ...
        for host in result:
            if host["profiling_status"] != "stopped":
                assert host["command_type"] in ["start", "stop"], "Active hosts should have valid command_type"
...
class TestProfilingHostStatusEdgeCases:
    ...
    def test_case_sensitivity(
        ...
        assert len(result_lower) == len(result_upper), "Case variations should return same results"
